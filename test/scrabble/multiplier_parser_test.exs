defmodule MultiplierParserTest do
  ...
  describe "MultiplierParser.parse/1" do
    ...
    test "DoubleWord" do
      ...
      assert {:error, {:invalid, :double_word}} =
               MultiplierParser.parse([%{"DoubleWord" => ["a"]}])
    end

    test "TripleWord" do
      assert {:ok, [:triple_word]} = MultiplierParser.parse([%{"TripleWord" => []}])

      assert {:error, {:invalid, :triple_word}} =
               MultiplierParser.parse([%{"TripleWord" => ["a"]}])
    end

    test "DoubleLetter" do
      assert {:ok, [double_letter: ~w(a b)]} =
               MultiplierParser.parse([%{"DoubleLetter" => ["a", "b"]}])

      assert {:ok, [double_letter: ~w(a b)]} =
               MultiplierParser.parse([%{"DoubleLetter" => ["A", "B"]}])

      assert {:ok, [double_letter: ~w(a)]} =
               MultiplierParser.parse([%{"DoubleLetter" => ["a", "鷹"]}])
    end

    test "TripleLetter" do
      assert {:ok, [triple_letter: ~w(a b)]} =
               MultiplierParser.parse([%{"TripleLetter" => ["a", "b"]}])

      assert {:ok, [triple_letter: ~w(a b)]} =
               MultiplierParser.parse([%{"TripleLetter" => ["A", "B"]}])

      assert {:ok, [triple_letter: ~w(a)]} =
               MultiplierParser.parse([%{"TripleLetter" => ["a", "鷹"]}])
    end
    ...
    test "Invalid key" do
      assert {:error, {:invalid_key, "Gibberish"}} =
               MultiplierParser.parse([%{"Gibberish" => ["blah"]}])
    end
